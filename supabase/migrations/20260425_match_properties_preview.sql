-- Função preview: corre o mesmo matching que `match_properties_for_negocio`
-- mas usa um conjunto de zonas em rascunho passado como argumento (em vez
-- das zonas persistidas no negócio). Permite live-update do mapa enquanto
-- o consultor desenha/adiciona zonas no picker, sem persistir.
...
BEGIN
  ...
  -- Replica a lógica do trigger: constrói geom em runtime a partir do JSONB
...
COMMENT ON FUNCTION match_properties_preview(uuid, jsonb) IS
  'Live preview do matching com zonas em rascunho. Mesmas regras de
   match_properties_for_negocio mas geom calculada on-the-fly a partir do JSONB.';
