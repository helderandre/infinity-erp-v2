...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        toast.success('Bem-vindo!', { description: 'A entrar no portal...' })
        fetch('/api/auth/record-login', { method: 'POST' }).catch(() => {})
        const redirect = new URLSearchParams(window.location.search).get('redirect')
        router.push(redirect || '/leads')
        router.refresh()
      }
    } catch (error) {
  ...
  return (
    <Card>
      <CardHeader className="space-y-1">
        <CardTitle className="text-2xl font-semibold tracking-tight">Portal de Parceiros</CardTitle>
