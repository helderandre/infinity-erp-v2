...
type StrengthLevel = { score: number; label: string; color: string; textColor: string }
...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        toast.error('Erro ao redefinir palavra-passe', { description: error.message })
        return
      }
      toast.success('Palavra-passe redefinida com sucesso!', { description: 'A entrar no portal...' })
      router.push('/leads')
      router.refresh()
    } catch {
  ...
  return (
    <Card>
      <CardHeader className="space-y-1">
        <CardTitle className="text-2xl font-semibold tracking-tight">Nova palavra-passe</CardTitle>
              ...
              <div className="space-y-2 pt-1">
                <div className="flex gap-1">
                  {[0, 1, 2, 3].map((i) => (
                    <div
                      key={i}
                      className={cn('h-1.5 flex-1 rounded-full transition-colors', i < level.score ? level.color : 'bg-muted')}
                    />
                  ))}
                </div>
