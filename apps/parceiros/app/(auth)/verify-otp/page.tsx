...
function VerifyOtpForm() {
  ...
  const handleVerify = useCallback(async (code: string) => {
    ...
    try {
      ...
      if (error) {
        toast.error('Código inválido', { description: 'O código introduzido está incorrecto ou expirou.' })
        setOtp('')
        return
      }
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      toast.success('Código reenviado!', { description: 'Verifique o seu email.' })
  ...
  const handleOtpChange = (value: string) => {
    setOtp(value)
    if (value.length === 8) handleVerify(value)
  }
  ...
  return (
    <Card>
      <CardHeader className="space-y-1">
        <CardTitle className="text-2xl font-semibold tracking-tight">Verificar código</CardTitle>
        <CardDescription>
      ...
      <CardContent>
        <div className="space-y-6">
          <div className="flex justify-center">
            <InputOTP maxLength={8} value={otp} onChange={handleOtpChange} disabled={loading}>
              <InputOTPGroup>
          ...
          <div className="flex items-center justify-center gap-1 text-sm">
            ...
            {resendCooldown > 0 ? (
              ...
              <Button variant="link" size="sm" className="h-auto p-0" onClick={handleResend} disabled={resending}>
                {resending ? <Spinner variant="infinite" size={12} className="mr-1" /> : <RefreshCw className="mr-1 h-3 w-3" />}
                Reenviar código
              </Button>
            )}
          </div>
