...
export function CourseEditorSkeleton({ embedded = false }: { embedded?: boolean }) {
  return (
    <div>
      {!embedded && <Skeleton className="h-40 w-full rounded-xl" />}
...
interface CourseEditorBodyProps {
  courseId: string
  /** When true, hides the page-level dark hero (the host Sheet provides the title). */
  embedded?: boolean
  /** Called after any mutation (save / publish / cover / content) so hosts can refresh lists. */
  onChanged?: () => void
}

export function CourseEditorBody({ courseId, embedded = false, onChanged }: CourseEditorBodyProps) {
  ...
  if (isLoading) return <CourseEditorSkeleton embedded={embedded} />
  if (!course) return (
  ...
  return (
    <div>
      ...
      {/* ─── Tab Navigation + (embedded) compact actions ─── */}
      <div className={cn('flex items-center justify-between gap-3', embedded ? '' : 'mt-6')}>
        ...
        {embedded && (
          <div className="flex items-center gap-2">
            ...
            {course.status === 'draft' && (
              <Button size="sm" className="rounded-full" onClick={handlePublish} disabled={isPublishing}>
                {isPublishing ? <Loader2 className="h-3.5 w-3.5 mr-1.5 animate-spin" /> : <Globe className="h-3.5 w-3.5 mr-1.5" />}
                Publicar
              </Button>
            )}
        ...
        <div key={activeTab} className="animate-in fade-in duration-300">
          ...
          {activeTab === 'content' && (
            <CourseBuilder
              ...
              onRefresh={() => { fetchCourse(); onChanged?.() }}
            />
          )}
