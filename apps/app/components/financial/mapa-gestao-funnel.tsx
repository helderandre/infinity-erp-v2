...
interface MapaGestaoFunnelProps {
  totals: MapaGestaoTotals
}

export function MapaGestaoFunnel({ totals }: MapaGestaoFunnelProps) {
  const layers = [
    { label: 'Comissoes Consultor', value: totals.split_total, width: 100, bg: 'bg-neutral-900', text: 'text-white' },
    { label: 'Rede', value: totals.network_total, width: 85, bg: 'bg-neutral-700', text: 'text-white' },
    { label: 'Agencia', value: totals.agency_total, width: 70, bg: 'bg-neutral-500', text: 'text-white' },
  ]
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="rounded-2xl border bg-card/50 backdrop-blur-sm p-4 text-center">
          <span className="text-xl font-bold tabular-nums">{totals.row_count}</span>
          <p className="text-xs text-muted-foreground mt-1">Total Linhas</p>
        </div>
      </div>
    </div>
  )
