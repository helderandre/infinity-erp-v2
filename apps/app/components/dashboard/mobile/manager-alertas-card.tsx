...
/**
 * Slide 2 of the manager mobile carousel — list of consultants with
 * red/amber export alerts. Tap a row to open the existing
 * ConsultantAlertsSheet (re-used so the action flow is identical to PC).
 */
...
import {
  ConsultantAlertsSheet, type ConsultantAlertRow,
} from '@/components/dashboard/consultant-alerts-sheet'

interface ManagerAlertasCardProps {
  fillViewport?: boolean
}

export function ManagerAlertasCard({ fillViewport }: ManagerAlertasCardProps) {
  ...
  return (
    <>
      <Card className={cardClass}>
        <div>
          <h3 className="text-base font-semibold tracking-tight">Alertas</h3>
          <p className="text-[11px] text-muted-foreground mt-0.5">
            {withAlerts.length === 0
              ? 'Todos os consultores em dia'
...
function ManagerAlertRow({
  row, onClick,
}: { row: ConsultantAlertRow; onClick: () => void }) {
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        'group w-full text-left rounded-2xl ring-1 p-3 transition-all',
        'hover:shadow-[0_4px_20px_-4px_rgb(0_0_0_/_0.08)]',
        row.red_count > 0
          ? 'ring-red-500/30 bg-red-500/[0.04] hover:ring-red-500/50'
          : row.yellow_count > 0
            ? 'ring-amber-500/30 bg-amber-500/[0.04] hover:ring-amber-500/50'
            : 'ring-border/40 bg-background/60 hover:ring-border/70',
      )}
    >
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          {hasAlerts ? (
            ...
            <p className="text-[11px] text-muted-foreground mt-0.5">Tudo em ordem</p>
          )}
        </div>
      </div>
    </button>
