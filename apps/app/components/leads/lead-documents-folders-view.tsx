...
interface LeadDocumentsFoldersViewProps {
  leadId: string
}

export function LeadDocumentsFoldersView({ leadId }: LeadDocumentsFoldersViewProps) {
  const { folders, isLoading, refetch } = useLeadDocuments(leadId)
  ...
  const handleBatchDownload = () =>
    downloadFromFolders({
      ...
      entityName: `lead-${leadId.slice(0, 8)}`,
  ...
  const openUploadFor = async (folder: DocumentFolder | null) => {
    ...
    try {
      const res = await fetch('/api/libraries/doc-types?applies_to=leads')
  ...
  const handleDelete = async (file: DocumentFile) => {
    const res = await fetch(`/api/leads/attachments/${file.id}`, { method: 'DELETE' })
    if (!res.ok) {
