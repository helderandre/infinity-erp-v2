-- Estende `match_properties_preview` com `p_localizacao_override` (text, default NULL).
--
-- Caso de uso: o picker do mapa abre dentro do diálogo de edição do
-- negócio. O utilizador pode ter escrito texto novo em `localizacao`
-- mas ainda NÃO ter guardado o negócio. A função preview lia o valor
-- antigo da DB e ignorava o texto em memória — sintoma percebido como
-- "o texto e a área desenhada cancelam-se".
--
-- Quando o override é fornecido (não-NULL e não-vazio), substitui o
-- valor da DB no `v_localizacao` usado no ramo de texto da OR.
