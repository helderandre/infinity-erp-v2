...
const UPLOAD_PATH = 'public/templates/email'
  ...
  try {
    ...
    if (!ALLOWED_TYPES.includes(file.type) && !ALLOWED_EXTENSIONS.includes(ext)) {
      return NextResponse.json(
        { error: 'Tipo de ficheiro não suportado. Use PDF, DOC, DOCX, XLS, XLSX, JPG, PNG ou WebP.' },
        { status: 400 }
      )
    }
    ...
    console.error('Erro ao fazer upload de anexo de template:', error)
    return NextResponse.json(
