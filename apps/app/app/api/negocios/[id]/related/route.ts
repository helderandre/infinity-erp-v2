...
export async function GET(
  ...
  try {
    ...
    const { data: negocioRow, error: negErr } = await adminDb
      ...
      .select(`
        *,
        ...
        property:dev_properties!negocios_property_id_fkey(id, title, external_ref, address_street, city, zone, address_parish, listing_price, property_type),
    ...
    // Gate: consultor só pode abrir o negócio se for o assigned_consultant_id
    // directo. Referenciações e splits ficam fora por agora. Esconde com
    // 404 (não revela existência). Gestão vê tudo.
    if (!isManagementRole(auth.roles)) {
      if (negocio.assigned_consultant_id !== auth.user.id) {
        return NextResponse.json({ error: 'Negócio não encontrado' }, { status: 404 })
      }
    }
    ...
    const { data: dealRows } = await adminDb
      ...
      .select(`
        ...
        external_property_type, external_property_construction_year, external_property_extra,
