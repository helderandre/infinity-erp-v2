...
Defines the lifecycle of a WhatsApp instance (`auto_wpp_instances`) in the ERP
— creation, connection, disconnection, deletion, user scoping, and the rules
that govern cleanup of derived data (`wpp_chats`, `wpp_messages`, `wpp_contacts`,
`wpp_labels`, `wpp_message_media`, `wpp_activity_sessions`,
`wpp_scheduled_messages`, `wpp_debug_log`) and R2-hosted media.
