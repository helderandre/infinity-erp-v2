...
- **THEN** response is 409 with `{ error, design_count: 7 }`, category unchanged
...
- **THEN** all 7 designs are moved to `outro` (templates get both `category` slug and `category_id` updated; personal designs get `category_id` updated), then the source category is deactivated, response 200 `{ ok: true, reassigned: 7 }`
