...
interface MarketingDesignCategoryDeleteDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  category: MarketingDesignCategory | null
  onDeleted?: () => void
}

export function MarketingDesignCategoryDeleteDialog({
  open,
  onOpenChange,
  category,
  onDeleted,
}: MarketingDesignCategoryDeleteDialogProps) {
  const { activeCategories, remove } = useMarketingDesignCategoriesContext()
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      toast.success(
        opts
          ? `Designs transferidos para "${targetOptions.find((c) => c.slug === opts.reassignTo)?.label ?? opts.reassignTo}" e categoria desactivada.`
          : 'Categoria eliminada'
      )
      onDeleted?.()
      resetState()
      ...
      const e = err as Error & { status?: number; designCount?: number }
      if (e.status === 409 && typeof e.designCount === 'number') {
        setDesignCount(e.designCount)
  ...
  return (
    <AlertDialog open={open} onOpenChange={handleOpenChange}>
      <AlertDialogContent className="rounded-2xl">
        <AlertDialogHeader>
          <AlertDialogTitle>
            {needsReassign ? 'Transferir designs' : 'Eliminar categoria'}
          </AlertDialogTitle>
          <AlertDialogDescription>
            {needsReassign ? (
              <>
