...
export function FunnelCoachSheet({
  ...
  // Auto-start conversation when sheet opens for a new context
  useEffect(() => {
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      ...
      >
          ...
          <div className="flex items-start justify-between gap-4">
            <SheetHeader className="p-0 gap-0 flex-1 min-w-0">
              ...
              <SheetDescription className="text-xs text-muted-foreground mt-1 truncate">
                {headerLabel ? `Diagnóstico · ${headerLabel}` : 'Diagnóstico em tempo real'}
              </SheetDescription>
            </SheetHeader>
            <Button
              variant="ghost"
        ...
        {/* Data quality issues — show as a banner before messages */}
        {dataIssues.length > 0 && (
          <div className="shrink-0 mx-4 mt-3 rounded-xl border border-amber-200 bg-amber-50/80 p-3 text-[11px] text-amber-900">
            <div className="flex items-start gap-2">
              <AlertTriangle className="h-3.5 w-3.5 shrink-0 mt-0.5" />
              <div className="flex-1">
                <div className="font-semibold mb-1">Problemas de dados a resolver</div>
          ...
          <div className="space-y-3">
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="bg-white border border-border/40 rounded-2xl px-3.5 py-2 shadow-sm text-xs text-muted-foreground">
                  A pensar...
                </div>
              </div>
            )}
