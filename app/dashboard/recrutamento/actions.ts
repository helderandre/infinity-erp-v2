...
export async function getEmailTemplates(): Promise<{ templates: RecruitmentEmailTemplate[]; error: string | null }> {
  const admin = createAdminClient()
  const { data, error } = await (admin as any).from("recruitment_email_templates")
    .select("*")
    .order("created_at", { ascending: true })

  if (error) return { templates: [], error: error.message }
  return { templates: (data ?? []) as RecruitmentEmailTemplate[], error: null }
}

export async function updateEmailTemplate(
  id: string,
  updates: Partial<RecruitmentEmailTemplate>
): Promise<{ success: boolean; error: string | null }> {
  const admin = createAdminClient()
  const { id: _, slug: __, created_at: ___, ...clean } = updates as any
  const { error } = await (admin as any).from("recruitment_email_templates")
...
export async function createEmailTemplate(
  data: { slug: string; name: string; description?: string; subject: string; body_html: string; from_email?: string; from_name?: string; variables?: string[] }
): Promise<{ template: RecruitmentEmailTemplate | null; error: string | null }> {
  const admin = createAdminClient()
  const { data: tpl, error } = await (admin as any).from("recruitment_email_templates")
    .insert({
      slug: data.slug,
      name: data.name,
      ...
      from_email: data.from_email || 'geral@infinitygroup.pt',
      from_name: data.from_name || 'Infinity Group',
  ...
  if (error) return { template: null, error: error.message }
  return { template: tpl as RecruitmentEmailTemplate, error: null }
}

export async function deleteEmailTemplate(id: string): Promise<{ success: boolean; error: string | null }> {
  const admin = createAdminClient()
  const { error } = await (admin as any).from("recruitment_email_templates").delete().eq("id", id)
  if (error) return { success: false, error: error.message }
  return { success: true, error: null }
}
