...
export default function StockPage() {
  ...
  const { stock, movements, loading, alertsOnly, setAlertsOnly, adjustStock, fetchMovements, refetch } =
    useEncomendaStock()
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Gestao de Stock</h1>
          <p className="text-muted-foreground">
            Controlo de inventario e movimentos de stock
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <Label htmlFor="alerts-only" className="text-sm">
          Apenas alertas (stock abaixo do minimo)
        </Label>
      </div>
      ...
      {loading ? (
        ...
        <EmptyState
          icon={Package}
          title="Nenhum registo de stock encontrado"
          description={
            alertsOnly
              ? 'Nao existem produtos com stock abaixo do minimo'
              : 'Ainda nao existem registos de stock'
          }
        />
      ) : (
        <StockTable
