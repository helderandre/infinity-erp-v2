  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight">Objetivos</h1>
          <p className="text-xs sm:text-sm text-muted-foreground mt-0.5">
            Quadro de objetivos multi-temporal dos consultores
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <Button variant="outline" size="sm" asChild className="rounded-full h-8 text-xs flex-1 sm:flex-initial min-w-0">
            <Link href="/dashboard/objetivos/relatorio-semanal">
              <FileText className="mr-1.5 h-3.5 w-3.5 shrink-0" />
              <span className="truncate">Relatório<span className="hidden sm:inline"> Semanal</span></span>
            </Link>
          </Button>
          <Button variant="outline" size="sm" asChild className="rounded-full h-8 text-xs flex-1 sm:flex-initial min-w-0">
            <Link href="/dashboard/objetivos/equipa">
              <UsersRound className="mr-1.5 h-3.5 w-3.5 shrink-0" />
              <span className="truncate">Equipa</span>
            </Link>
          </Button>
      ...
      {!isLoading && goals.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
          <div className="rounded-2xl bg-orange-500 text-white p-4 sm:p-5 shadow-sm">
            ...
            <p className="text-2xl sm:text-3xl font-bold truncate">{formatCurrency(totalAnnual)}</p>
          ...
          <div className="rounded-2xl border bg-white p-4 sm:p-5 shadow-sm">
            ...
            <p className="text-2xl sm:text-3xl font-bold">{avgSellers}%</p>
          ...
          <div className="rounded-2xl border bg-white p-4 sm:p-5 shadow-sm">
            ...
            <p className="text-2xl sm:text-3xl font-bold">{avgBuyers}%</p>
            <p className="text-xs text-muted-foreground mt-1">do objetivo total</p>
          </div>
        </div>
      )}
            ...
            {goals.map((goal) => {
              ...
              return (
                <Link
                  ...
                  className="flex items-center gap-3 sm:gap-4 px-4 sm:px-6 py-4 hover:bg-muted/30 transition-colors group"
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs font-semibold tabular-nums mt-1 sm:hidden">
