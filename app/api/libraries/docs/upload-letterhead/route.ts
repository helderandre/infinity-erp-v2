...
const ALLOWED_EXTENSIONS = ['doc', 'docx', 'jpg', 'jpeg', 'png', 'webp']
const MAX_SIZE = 10 * 1024 * 1024 // 10MB
const UPLOAD_PATH = 'public/templates/docs/letterheads'
  ...
  try {
    ...
    if (!ALLOWED_TYPES.includes(file.type) && !ALLOWED_EXTENSIONS.includes(ext)) {
      return NextResponse.json(
        { error: 'Tipo de ficheiro não suportado. Use DOC, DOCX ou imagem.' },
        { status: 400 }
      )
    }
    ...
    console.error('Erro ao fazer upload de timbrado:', error)
