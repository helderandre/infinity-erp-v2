...
/**
 * POST /api/perfil/photo — upload own profile photo
 */
export async function POST(request: Request) {
  try {
    ...
    const key = `public/usuarios-fotos/${userId}/${Date.now()}-${sanitized}`
    ...
    try {
      ...
      if (nobgBuffer) {
        const nobgKey = `public/usuarios-fotos/${userId}/${Date.now()}-nobg-${sanitized.replace(/\.\w+$/, '.png')}`
        await s3.send(
          new PutObjectCommand({
            Bucket: R2_BUCKET,
            Key: nobgKey,
