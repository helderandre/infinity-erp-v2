...
export async function POST(request: Request) {
  try {
    ...
    if (file.size > 2 * 1024 * 1024) {
      return NextResponse.json({ error: 'Ficheiro excede 2MB.' }, { status: 400 })
    }

    const buffer = Buffer.from(await file.arrayBuffer())
    const ext = file.name.split('.').pop()?.toLowerCase() || 'webp'
    const key = `calendario/${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${ext}`
    ...
    console.error('[calendar/upload-cover]', err)
